...
function initData() {
    ...
    if (savedHorarios !== null) {
        ...
        State.setHorarios([
            { id: 1, data: today, horario: '08:00' },
            { id: 2, data: today, horario: '09:00' },
            { id: 3, data: today, horario: '10:00' },
            { id: 4, data: today, horario: '13:00' },
            { id: 5, data: today, horario: '14:00' },
            { id: 6, data: today, horario: '15:00' },
            { id: 7, data: today, horario: '16:00' },
            { id: 8, data: today, horario: '17:00' },
            { id: 9, data: today, horario: '18:00' },
            { id: 10, data: today, horario: '19:00' },
            { id: 11, data: tomorrow, horario: '08:00' },
            { id: 12, data: tomorrow, horario: '09:00' },
            { id: 13, data: tomorrow, horario: '10:00' },
            { id: 14, data: tomorrow, horario: '11:00' },
            { id: 15, data: tomorrow, horario: '13:00' },
            { id: 16, data: tomorrow, horario: '14:00' },
            { id: 17, data: tomorrow, horario: '15:00' },
            { id: 18, data: tomorrow, horario: '16:00' },
            { id: 19, data: tomorrow, horario: '17:00' },
            { id: 20, data: tomorrow, horario: '18:00' },
            { id: 21, data: tomorrow, horario: '19:00' }
        ]);
        persistHorarios();
    }
}
...
// Função para lidar com o clique nos serviços
function setupServicoClickHandler() {
    ...
    servicoCards.forEach((card, index) => {
        ...
        newCard.addEventListener('click', (e) => {
            ...
            if (servico) {
                ...
                showClientePage('agendar');
                // Selecionar o serviço no dropdown
                const servicoSelect = document.getElementById('servicoSelect');
                if (servicoSelect) {
                    servicoSelect.value = servico.id;
                }
                // Mostrar toast de confirmação
                Toast.show(`"${servico.nome}" selecionado! Agora escolha a data e horário`, 'success');
            }
        });
    });
}
...
function renderServicos() {
    ...
    // Adicionar os event listeners após renderizar
    setTimeout(() => setupServicoClickHandler(), 0);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const today = DateUtils.today();
    const novaDataHorario = document.getElementById('novaDataHorario');
    const dataAgendamento = document.getElementById('dataAgendamento');
    
    if (novaDataHorario) novaDataHorario.min = today;
    if (dataAgendamento) dataAgendamento.min = today;
    ...
    if (loginEmail) loginEmail.addEventListener('keydown', e => e.key === 'Enter' && handleLogin());
    if (loginPassword) loginPassword.addEventListener('keydown', e => e.key === 'Enter' && handleLogin());

    const clienteLogout = document.getElementById('clienteLogout');
    const barbeiroLogout = document.getElementById('barbeiroLogout');
    
    if (clienteLogout) clienteLogout.addEventListener('click', logout);
    if (barbeiroLogout) barbeiroLogout.addEventListener('click', logout);

    const dataAgendamentoInput = document.getElementById('dataAgendamento');
    if (dataAgendamentoInput) dataAgendamentoInput.addEventListener('change', onDataChange);
    
    const btnAgendar = document.getElementById('btnAgendar');
    if (btnAgendar) btnAgendar.addEventListener('click', handleAgendamento);
    
    const btnAddHorario = document.getElementById('btnAddHorario');
    if (btnAddHorario) btnAddHorario.addEventListener('click', addHorarioDisponivel);

    const clienteScreen = document.getElementById('clienteScreen');
    if (clienteScreen) {
        clienteScreen.addEventListener('click', e => {
    ...
    const barbeiroScreen = document.getElementById('barbeiroScreen');
    if (barbeiroScreen) {
        barbeiroScreen.addEventListener('click', e => {
            const btn = e.target.closest('.nav-item[data-page]');
            if (!btn) return;
    ...
    const clienteMenuToggle = document.getElementById('clienteMenuToggle');
    const barbeiroMenuToggle = document.getElementById('barbeiroMenuToggle');
    const clienteOverlay = document.getElementById('clienteOverlay');
    const barbeiroOverlay = document.getElementById('barbeiroOverlay');
    
    if (clienteMenuToggle) clienteMenuToggle.addEventListener('click', () => toggleSidebar('clienteSidebar', 'clienteOverlay'));
    if (barbeiroMenuToggle) barbeiroMenuToggle.addEventListener('click', () => toggleSidebar('barbeiroSidebar', 'barbeiroOverlay'));
    if (clienteOverlay) clienteOverlay.addEventListener('click', () => closeSidebar('clienteSidebar', 'clienteOverlay'));
    if (barbeiroOverlay) barbeiroOverlay.addEventListener('click', () => closeSidebar('barbeiroSidebar', 'barbeiroOverlay'));

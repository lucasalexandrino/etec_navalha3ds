(function () {
  ...
  function getSessao() {
    return load(KEY_SESSAO, null);
  }

  function setSessao(s) {
    if (s) save(KEY_SESSAO, s);
  ...
  function getOpenCloseMinutes(dayOfWeek) {
    ...
    if (dayOfWeek >= 1 && dayOfWeek <= 6) {
      return { openMin: 7 * 60, closeMin: 19 * 60 };
    }
    return null;
  }
  ...
  function gerarHorariosDisponiveis(
    ...
    for (var t = openMin; t <= maxStartMin; t += SLOT_STEP_MIN) {
      ...
      // Respeitar configuração personalizada do barbeiro (ignorar horários desmarcados)
      if (!isHorarioAtivo(barberId, timeStr)) continue;
  ...
  function applyScreen(role) {
    ...
    if (clienteScreen) clienteScreen.classList.add("hidden");
    if (barbeiroScreen) barbeiroScreen.classList.add("hidden");
  ...
  function renderMeusAgendamentos() {
    ...
    ags.forEach(function (a) {
      ...
      card.innerHTML =
        ...
        "</div>" +
        '<div class="agendamento-pagamento">' +
  ...
  function getHorariosCustom() {
    var c = load(KEY_HORARIOS_CUSTOM, {});
    return typeof c === "object" ? c : {};
  }

  function setHorarioCustom(barberId, timeStr, active) {
    var c = getHorariosCustom();
    if (!c[barberId]) c[barberId] = {};
    c[barberId][timeStr] = active;
    save(KEY_HORARIOS_CUSTOM, c);
  }

  function isHorarioAtivo(barberId, timeStr) {
    var c = getHorariosCustom();
    if (!c[barberId]) return true; // Padrão: todos ativos se não houver configuração
    if (typeof c[barberId][timeStr] === "boolean") return c[barberId][timeStr];
  ...
  function renderHorariosBarbeiro() {
    ...
    var barberId = sessao && sessao.barberId;
    
    if (!barberId) return;

    var headerHtml = 
      '<div class="empty-state" style="text-align:left; margin-bottom: 1.5rem;">' +
      "<p><strong>Olá, " + escapeHtml(nome) + ".</strong></p>" +
      "<p>Abaixo estão os horários da grade padrão (07h às 19h, intervalos de 15min). " +
      "Desmarque os horários em que você <strong>NÃO</strong> deseja receber agendamentos. " +
      "As alterações são salvas automaticamente.</p>" +
      "</div>";

    // Gerar lista de todos os horários possíveis do dia (apenas para exibição/configuração)
    var slots = [];
    var openMin = 7 * 60;
    var closeMin = 19 * 60;
    for (var t = openMin; t <= closeMin; t += SLOT_STEP_MIN) {
      var h = Math.floor(t / 60);
      var m = t % 60;
      var timeStr = pad2(h) + ":" + pad2(m);
      
      // Simular almoço bloqueado visualmente (ou permitir que o barbeiro mude, mas vamos seguir a regra de bloquear 12-13h por padrão, mas se for config, melhor deixar tudo e ele escolhe)
      // Mantendo a regra de almoço fixa:
      if (h === 12) continue;
      
      slots.push(timeStr);
    }

    var gridHtml = '<div class="horarios-grid">';
    slots.forEach(function(timeStr) {
      var active = isHorarioAtivo(barberId, timeStr);
      var checked = active ? "checked" : "";
      var cardClass = active ? "horario-slot-card" : "horario-slot-card disabled";
      
      gridHtml += 
        '<div class="' + cardClass + '" data-time="' + timeStr + '">' +
          '<span><i class="fas fa-clock"></i> ' + timeStr + '</span>' +
          '<label class="toggle-switch">' +
            '<input type="checkbox" class="horario-toggle" data-time="' + timeStr + '" ' + checked + '>' +
    ...
    var toggles = el.querySelectorAll(".horario-toggle");
    toggles.forEach(function(toggle) {
      toggle.addEventListener("change", function(e) {
        var tStr = e.target.getAttribute("data-time");
        var isAtivo = e.target.checked;
        setHorarioCustom(barberId, tStr, isAtivo);
        
        var card = e.target.closest(".horario-slot-card");
        if (isAtivo) {
        ...
        showToast("Horário " + tStr + (isAtivo ? " ativado" : " desativado") + " com sucesso.", "success");
      });
    });
  }
  ...
  function renderTodosAgendamentos() {
    ...
    var agora = new Date();
    var hojeY = agora.getFullYear();
    var hojeM = agora.getMonth();
    var hojeD = agora.getDate();

    ags.sort(function (a, b) {
      var dA = new Date(a.startIso);
      var dB = new Date(b.startIso);
      
      var isHojeA = dA.getFullYear() === hojeY && dA.getMonth() === hojeM && dA.getDate() === hojeD;
      var isHojeB = dB.getFullYear() === hojeY && dB.getMonth() === hojeM && dB.getDate() === hojeD;
      
      if (isHojeA && !isHojeB) return -1;
      if (!isHojeA && isHojeB) return 1;
      
      var timeA = dA.getTime() || 0;
      var timeB = dB.getTime() || 0;
      return timeA - timeB;
    });
